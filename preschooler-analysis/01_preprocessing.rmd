...
tidy_all_rt_task_data <- function(raw_data){
  ...
  cleaned_rt_data <- cleaned_rt_data%>%
    ...
    select(subject, block_number, trial_number, block_type, trial_type, block_background, block_deviant, deviant_position,stimulus_displayed, trial_looking_time)
...
```{r}
df <- tidy_all_rt_task_data(raw_df)
