...
prolific_path <- here("data/merged_data/prolific_demog.csvß")
...
df.temp_memory_answer <- left_join(df.temp_processed, df.temp_memory, by = "subject_block") %>% 
  mutate(
    memory_question_type = case_when(
     (memory_question_stimuli == block_background  | memory_question_stimuli == block_deviant) ~ "true_stimulus",
       (memory_question_stimuli != block_background  && memory_question_stimuli != block_deviant) ~ "false_stimulus", 
     TRUE ~ "error!"
    ),
