...
```{r}
lp_prior <- dbeta(x = seq(0, 1, 0.2), shape1 = 1, shape2 = 1, log = TRUE)
lp_posterior <- dbeta(x = seq(0, 1, 0.2), shape1 = 2, shape2 = 1, log = TRUE)

sum(lp_prior * exp(lp_prior))
sum(lp_posterior * exp(lp_posterior))
```
...
If we want to calculate the $EIG(z_{t+1})$

We will need to do: 

$$EIG(z_{t+1}) = \sum_{z_{t+1}\in[1, 0]}p(z_t+1|\theta_{t})IG(z_{t+1})$$
in words: expected information gain of observation at time point t+1 is the information gain of t+1 weighted by the probability of such observation given the prior (aka the posterior at time point t+1). 
we got stuck by the term $$p(z_{t+1})$$ here. it doesn't feel so reasonable to use the bayes rule again? but also we shouldn't really assume that the probabiity is equal among two possibilities? did we miss something here? 
