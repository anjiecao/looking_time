...
```{r}
library(lme4)
model <- lm(trial_looking_time ~ trial_number, data = clean_rt_df)
...
clean_rt_df %>% 
  ...
  ggplot(aes(x = rating, y = log(trial_looking_time), color = stimulus_type)) + 
  geom_point() + 
  geom_smooth() + 
  facet_grid(stimulus_type~subject_type)
