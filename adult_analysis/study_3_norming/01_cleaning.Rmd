...
similarity_comparison <- raw_d %>% 
  ...
  select(subject, rt, response, target, target_animacy, target_pose, target_number, left, left_animacy, left_pose, left_number, right_animacy, right_pose, right_number, 
         comparison_type) %>% 
  separate(comparison_type, into = c("left_violation", "right_violation"), sep = "_") %>% 
  select(subject, rt, response, target, left_violation, right_violation) %>% 
