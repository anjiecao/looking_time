...
e_dis <- read_csv(here("data/cosine_distances_with_number.csv"))%>% rename(stim_name = `...1`)
...
dist_plot <- ds_proportion_df %>% 
  ...
  mutate(
    unchosen_p_eds = ifelse(chosen_violation == left_violation, right_p_eds, left_p_eds)
  ) %>% 
  ggplot(aes(x = violation_type, y = unchosen_p_eds, color = chosen_violation)) + 
  stat_summary(fun.data = "mean_cl_boot", position = position_dodge(width = .2)) + 
  ylim(0, 1) + 
  theme_few() + 
