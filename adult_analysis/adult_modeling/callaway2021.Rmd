...
```{r}
example_d <- example %>% 
  select(time_point, belief, variance)
example_d
```

```{r}
densities <- apply(example_d[, -1], 1, function(x) rnorm(n = 1000, mean = x[1], sd = x[2]))
colnames(densities) <- example_d$time_point

plot(0, type = "n", xlim = c(min(densities), max(densities)), ylim = c(0, 10))
...
example_d %>% 
  ggplot(aes(x = time_point), 
         y = belief) + 
  geom_pointrange(aes(y = belief, ymin = belief + variance, ymax = belief - variance))
```
