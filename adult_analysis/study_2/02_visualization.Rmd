...
d_0 %>% 
  select(trial_number, trial_type, block_type, trial_looking_time) %>% 
...
model_1 <- lmerTest::lmer(log(trial_looking_time) ~ I(exp(1)**(-trial_number)) * trial_type * block_type * forced_exposure_time  + (1| subject),
                          data = d %>% mutate(forced_exposure_time = as.character(forced_exposure_time)) %>% 
                            filter(trial_number > 1))
summary(model_1)
