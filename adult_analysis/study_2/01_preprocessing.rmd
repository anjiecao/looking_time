...
df_sim <- tibble("exclude_reason" = rep("similarity", length(similarity_sbj)), 
                     sbj = similarity_sbj)

df_complex <- tibble("exclude_reason" = rep("complexity", length(complexity_sbj)), 
                     sbj = complexity_sbj)
```
...
```{r}
participants_exclusion <- bind_rows(df_low_var, df_sim, df_complex, df_prolific)
