...
d %>% 
  ...
  ggplot(aes(x = age, y = log(trial_looking_time))) + 
  geom_point() +
...
d %>% 
  ...
  ggplot(aes(x = age, y = task_question_response, 
             color = task_question_type), 
         group = task_question_type) + 
...
```{r}
sim_d <- read_csv(here("data/02_processed_data/processed_similarity_data.csv"))

sim_d <- sim_d %>% 
  rowwise() %>%
  mutate(stim_distance =  lookup_distance(stimulus_left, stimulus_right, em_d))
```

```{r}
sim_d %>% 
  ggplot(aes(x = stim_distance, y = rating)) + 
  geom_jitter(alpha = .2) + 
  geom_smooth(method = "lm")
```
...
```{r}
em_d %>% 
  arrange(complex_01_A_a.png) %>% 
  pull(complex_01_A_a.png)
```
...
```{r}
dist_d <- dist_d %>% 
  mutate(bd_distance_centered = case_when(
    trial_type == "background" ~ 0, 
    trial_type == "deviant" ~ bd_distance
  ))
```
...
model_1 <- lmerTest::lmer(log(trial_looking_time) ~ I(exp(1)**(-trial_number)) * trial_type * block_type + (1 | subject), data = dist_d)
summary(model_1)

model_with_dist <-  lmerTest::lmer(log(trial_looking_time) ~ I(exp(1)**(-trial_number)) * trial_type * block_type +  trial_type * bd_distance_centered + (1 | subject), data = dist_d)

anova(model_with_dist, model_1)
```
...
dist_d %>% 
  ...
  labs(title = "diff to baseline")
  
```

```{r}
dist_d_lt %>% 
  ggplot(aes(x = deviant_position, 
             y = log(bd_diff), 
             color = block_type), 
         group = block_type) + 
  geom_point(alpha = .1, position = position_jitter(width = .2)) + 
  stat_summary(fun.data = "mean_cl_boot", 
               position = position_dodge(width = .5)) + 
  facet_wrap(~block_number) + 
  labs(title = "diff to previous trial")
```
...
```{r}
em_d %>% 
  pivot_longer(cols = -stimulus, 
               names_to = "distance_to") %>% 
  mutate(
    s_complexity = case_when(
      grepl("complex", stimulus) ~ "complex", 
      grepl("simple", stimulus) ~ "simple", 
      TRUE ~ "unity"
    ), 
    distance_to_complexity = case_when(
      grepl("complex", distance_to) ~ "complex", 
      grepl("simple", distance_to) ~ "simple",
       TRUE ~ "unity"
    )
  ) %>% 
  mutate(
    distance_pair_complexity = case_when(
      s_complexity == "complex" & distance_to_complexity == "complex" ~ "c_c",
      s_complexity == "complex" & distance_to_complexity == "simple" ~ "c_s",
      #s_complexity == "simple" & distance_to_complexity == "complex" ~ "s_c",
      s_complexity == "simple" & distance_to_complexity == "simple" ~ "s_s", 
      s_complexity == "simple" & distance_to_complexity == "unity" ~ "s_u", 
      #s_complexity == "unity" & distance_to_complexity == "simple" ~ "u_s", 
      s_complexity == "complex" & distance_to_complexity == "unity" ~ "c_u",
      #s_complexity == "unity" & distance_to_complexity == "complex" ~ "u_c",
      s_complexity == "unity" & distance_to_complexity == "unity" ~ "u_u"
    )
  ) %>% 
  filter(!is.na(distance_pair_complexity)) %>% 
  filter(stimulus != distance_to) %>% 
  ggplot(aes(x = distance_pair_complexity, 
             y = value)) + 
  geom_violin() + 
  stat_summary(fun.data = "mean_cl_boot") + 
  theme_classic()
...
d %>% 
  ...
  ylab("log RT (seconds)") + 
  xlab("block Number") + 
  labs(title = "First trial in each block")
```
...
```{r}
dv_lt <- d %>% 
  select(subject, block_type, block_number, trial_number, deviant_position, trial_looking_time) %>% 
  filter(trial_number == deviant_position) %>% 
  rename(deviant_looking_time = trial_looking_time) %>% 
  select(subject, block_type, block_number, deviant_position, deviant_looking_time) 

bd_lt <- d %>% 
  select(subject,block_type,  block_number, trial_number, deviant_position, trial_looking_time) %>% 
  filter(trial_number == deviant_position-1) %>% 
  rename(bg_looking_time = trial_looking_time) %>% 
  select(subject,block_type, block_number, deviant_position, bg_looking_time) 


dishab_lt_df <- dv_lt %>% 
  left_join(bd_lt, by = c("subject", "block_type", "block_number", "deviant_position")) %>% 
  mutate(dishab_lt = deviant_looking_time - bg_looking_time)

dishab_lt %>% 
  ggplot(aes(x=block_number, y=log(dishab_lt), colour=block_type)) + 
  stat_summary(fun.data = "mean_cl_boot", position = position_dodge(width = .2)) + 
  #geom_smooth() + 
  #langcog::theme_mikabr() +
  theme_classic()+
  ...
  labs(title = "deviant - background")

```

## plotting all trials together 

```{r}
d %>% 
  distinct(subject, block_number, deviant_position, block_type) %>% 
  mutate(block_identifier = paste(block_number,deviant_position, sep = "_")) %>% 
  select(subject) %>%
  group_by(subject) %>% 
  pivot_wider(block_identifier, names_from)
...
```{r}
d %>% 
  mutate(total_trial_number = ((block_number-1) * 6) + trial_number) %>% 
  group_by(subject) %>% 
  count(total_trial_number) %>% 
  ggplot(aes(x = total_trial_number))
