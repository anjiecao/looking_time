...
lt_d <- lt_d %>% 
  ...
  mutate(
    violation_type = case_when(
      # animacy violation
      same_animacy == FALSE & same_number == TRUE & same_pose == TRUE & same_id == FALSE ~ "animacy", 
      # pose violation 
...
trimmed_lt_d <- lt_d %>% 
  filter(!(subject %in% participants_exclusion$sbj))
...
# try some data exploration on the datasets

```{r}
write_csv(final_trimmed_lt_d, here("data/01_trimmed_data/corrected_lt_d.csv"))
```
