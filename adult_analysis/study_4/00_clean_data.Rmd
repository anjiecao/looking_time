...
d <- read.csv(here("data/00_raw_data/full_raw.csv")) %>% filter(prolific_id != "") %>% filter(!prolific_id %in% exclude_d)
```
...
```{r}
prolific_d <- read_csv(here("data/00_raw_data/prolific_info.csv"))
...
tidy_d <- trimmed_d %>% 
  ...
  filter(background != "") %>% 
  filter(!is.na(rt)) %>% 
  select(prolific_id, rt, block_type, fam_trial, background, deviant) 

write_csv(tidy_d, here("data/01_clean_data/tidy_d.csv"))
```
