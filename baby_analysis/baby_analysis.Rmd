...
# remove outlier looking times
 # complete_df <- complete_df %>% filter(looking_time_2000 > 2)
...
comparison_df <- complete_df %>% filter(coder_num == 2) %>% 
  pivot_wider(names_from = coder, values_from = starts_with('look')) %>% 
  filter(abs(looking_time_2000_1 - looking_time_2000_2) > 4) %>% select(block_number, fam_or_test, subject_id)
...
x <- complete_df %>% filter(fam_or_test == 'test' & coder == 1) %>% dplyr::group_by(block_number, block_type, fam_duration) %>% dplyr::summarize(n = n())

# check which rotation of 3,5,7,7,5,3 would balance order the best

rotation = c(3,7,5,5,7,3)
