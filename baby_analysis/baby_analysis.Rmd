...
```{r}
# removelooking times less than 2 sec according to coder 1
...
# Big discrepancies (more than 4 sec)
 View(wide_df %>% filter(coder_num == 2  & abs(looking_time_2000_1 - looking_time_2000_2) > 5))
...
for (i in 1:nrow(all_combs)) {
    ...
    for (k in 1:length(fam_durations_new)) {
        ...
        # increment by one
          hypothetical_df$n_trial[
            ...
            #hypothetical_df %>% 
        #tibble::add_row(fam_duration = fam_durations_new[k], 
             #   block_type = block_types_new[k], 
             #   subject_id = 30 + j, 
             #   block_number = k)
  ...
  if (i == 8) {
    store_df = hypothetical_df 
  }
  
  if (i == 200) {
    store_df2 = hypothetical_df
  }
  
    if (i == 122) {
    store_df3 = hypothetical_df
  }
}
...
store_df %>% ggplot(aes(x=block_number, y=n_trial, group=block_type, color = block_type)) + geom_point(alpha=.8, position = position_dodge(width = .1))  + 
  geom_line() + facet_wrap(~fam_duration) + ggtitle('minimized')

store_df2 %>% ggplot(aes(x=block_number, y=n_trial, group=block_type, color = block_type)) + geom_point(alpha=.8, position = position_dodge(width = .1))  + 
  geom_line() + facet_wrap(~fam_duration) + ggtitle('random other one')

store_df3 %>% ggplot(aes(x=block_number, y=n_trial, group=block_type, color = block_type)) + geom_point(alpha=.8, position = position_dodge(width = .1))  + 
  geom_line() + facet_wrap(~fam_duration) + ggtitle('random other one')
...
# calculate means
mean_df <- summarySE(test_df, measurevar="normalized_LT", groupvars=c("block_number"))

ggplot(test_df %>% filter(fam_or_test == 'test'), aes(x=block_number, y = normalized_LT)) + 
  geom_point(alpha = 0.2) + geom_line(aes(group = subject_id), alpha=0.2) + stat_summary(fun.data = "mean_cl_boot", geom = "line", position = position_dodge(width = .1))  + stat_summary(fun.data = "mean_cl_boot", geom = "pointrange", position = position_dodge(width = .1)) + xlab('Block Number') + ylab('normalized LT') + theme_classic(base_size = 15)
```
...
# calculate means
mean_df <- summarySE(test_df, measurevar="normalized_LT", groupvars=c("block_type"))

ggplot(test_df, aes(x=block_type, y=normalized_LT)) + geom_jitter(width = 0.1, alpha = 0.2)  + stat_summary(fun.data = "mean_cl_boot", geom = "pointrange", position = position_dodge(width = .1)) + xlab('Test trial type') + ylab('Looking Time (s)') + theme_classic(base_size = 15)
```
