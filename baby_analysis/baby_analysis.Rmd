...
looking_df <- looking_df %>% filter(trial_type != 'a') %>% 
  ...
  dplyr::rename(looking_time_total = looks_on_total_s,
                looking_time_until_lookaway = looks_on_before_lookaway_s) %>%
...
```{r}
# Correlation
coder_1 <- complete_df %>% filter(coder_num == 2 & coder == 1) %>% pull(looking_time_until_lookaway)
coder_2 <- complete_df %>% filter(coder_num == 2 & coder == 2) %>% pull(looking_time_until_lookaway)
...
test_df <- complete_df %>% filter(fam_or_test == 'test' & coder == 1) 


# calculate means
mean_df <- summarySE(test_df, measurevar="looking_time_until_lookaway", groupvars=c("block_number"))

ggplot(test_df %>% filter(fam_or_test == 'test'), aes(x=block_number, y = looking_time_until_lookaway)) + 
  geom_point(alpha = 0.2) + geom_line(aes(group = subject_id), alpha=0.2) + stat_summary(fun.data = "mean_cl_boot", geom = "line", position = position_dodge(width = .1))  + stat_summary(fun.data = "mean_cl_boot", geom = "pointrange", position = position_dodge(width = .1)) 
```
...
# calculate means
mean_df <- summarySE(test_df, measurevar="looking_time_until_lookaway", groupvars=c("block_type"))

ggplot(test_df, aes(x=block_type, y=looking_time_until_lookaway)) + geom_point() + geom_point(data=mean_df, aes(x = block_type, y=looking_time_until_lookaway), fill='red', shape=24, size=3) + geom_errorbar(data=mean_df, aes(ymin=looking_time_until_lookaway-ci/2, ymax=looking_time_until_lookaway+ci/2), width=0.2, size=0.7, color="red")
```

## Looking time by familiarization duration
```{r}
# calculate means
mean_df <- summarySE(test_df, measurevar="looking_time_until_lookaway", groupvars=c("fam_duration"))

ggplot(test_df, aes(x=fam_duration, y=looking_time_until_lookaway)) + geom_point() + geom_point(data=mean_df, aes(x = fam_duration, y=looking_time_until_lookaway), fill='red', shape=24, size=3) + geom_errorbar(data=mean_df, aes(ymin=looking_time_until_lookaway-ci/2, ymax=looking_time_until_lookaway+ci/2), width=0.2, size=0.7, color="red")
...
```{r}
# calculate means
mean_df <- summarySE(test_df, measurevar="looking_time_until_lookaway", groupvars=c("block_type", "fam_duration"))

ggplot(test_df, aes(x=block_type, y=looking_time_until_lookaway, group=fam_duration, color = fam_duration)) + geom_point(alpha=0.2, position = position_dodge(width = .1)) + 
    stat_summary(fun.data = "mean_cl_boot", geom = "line", position = position_dodge(width = .1))  + 
  stat_summary(fun.data = "mean_cl_boot", geom = "pointrange", position = position_dodge(width = .1)) + facet_wrap(~fam_duration) + theme_classic() 
...
model1 <- lme(log(looking_time_until_lookaway+0.01) ~ block_type * fam_duration * block_number, random = ~1|subject_id, data = test_df)
anova(model1)
