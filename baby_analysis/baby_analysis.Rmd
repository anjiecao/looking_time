...
trial_df <- trial_df %>% complete(subject_id, nesting(block_number, fam_or_test)) %>%
  # add temp_id
  mutate(temp_id = paste0(subject_id, block_number, fam_or_test))
  

# add temp id to looking df
looking_df_new <- looking_df %>% filter(trial_type != 'a') %>% 
  mutate(
  ...
  mutate(block_number = row_number(),
         ...
         temp_id = paste0(subject, block_number, trial_type)) %>%
  select(-c("block_number", "subject"))


# join trials and looking times
complete_df <- left_join(looking_df_new, trial_df, by = 'temp_id')  %>%
  select(-c("temp_id", "x1"))
