...
wide_df %>% filter(coder_num == 2  & abs(looking_time_2000_1 - looking_time_2000_2) > 5)
...
ggplot(test_df, aes(x=block_type, y=looking_time_2000_1, group=fam_duration, color = fam_duration)) + geom_point(alpha=0.2, position = position_dodge(width = .1)) + 
...
model1 <- lme(log(looking_time_2000_1+0.01) ~ block_type * fam_duration, random = ~1|subject_id, data = test_df %>% mutate(fam_duration = as.numeric(fam_duration)))
summary(model1)
