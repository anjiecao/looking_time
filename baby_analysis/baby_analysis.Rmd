...
```{r}
# calculate means
mean_df <- summarySE(test_df, measurevar="looking_time_2000", groupvars=c("block_type", "fam_duration"))

ggplot(test_df, aes(x=block_number, y=looking_time_2000, group=fam_duration, color = fam_duration)) + geom_point(alpha=0.2, position = position_dodge(width = .1)) + 
    stat_summary(fun.data = "mean_cl_boot", geom = "line", position = position_dodge(width = .1))  + 
  stat_summary(fun.data = "mean_cl_boot", geom = "pointrange", position = position_dodge(width = .1)) + facet_wrap(~fam_duration) + xlab('Block number') + ylab('LT (in sec)') + labs(color = '# of Fam Trials') + theme_classic() + 
  theme(strip.text.x = element_text(size = 14, colour = "black")) + ggtitle('Block number by trial type & fam duration')
