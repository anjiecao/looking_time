...
demog_info = read.csv('data/exp2/demographic_info_exp2.csv')
...
age_and_gender <- jspsych_df %>% select(responses) %>%
  ...
  select(age, gender, session_num) %>% mutate(subject_num = as.numeric(subject_num))
...
trial_df <- jspsych_df %>%
  ...
  ungroup() %>% left_join(age_and_gender, by = c('subject_num', 'session_num')) %>% left_join(demog_info, by = c('subject_num'))
...
```{r}
wide_df <- test_df %>% pivot_wider(names_from = session_number, values_from = looking_time_2000, names_prefix = 'session') %>% dplyr::group_by(subject_num) %>% dplyr::summarise(session1 = mean(session1, na.rm = TRUE), session2 = mean(session2, na.rm = TRUE)) 

ggplot(wide_df, aes(x = log(session1), y = log(session2))) + geom_point(size =4) + geom_smooth(method = 'lm') + xlab('session 1 LT (log s)')  + ylab('session 2 LT (log s)') + theme_classic(base_size = 20) + coord_fixed()

cor(wide_df$simple, wide_df$complex, use = 'na.or.complete')

cor.test(log(wide_df$session1), log(wide_df$session2), use = 'na.or.complete')
...
ggplot(demog_info, aes(x=income)) + geom_histogram(stat = 'count') + xlab('Income') + ylab('Frequency') + theme_classic() + theme(strip.text.x = element_text(size = 14, colour = "black"),
        axis.text=element_text(size=5),
        axis.title= element_text(size = 14)) +
  ggtitle('') 

ggplot(demog_info, aes(x=race)) + geom_histogram(stat = 'count') + xlab('Income') + ylab('Frequency') + theme_classic() + theme(strip.text.x = element_text(size = 14, colour = "black"),
        axis.text=element_text(size=5),
        axis.title= element_text(size = 14)) +
  ggtitle('') 
