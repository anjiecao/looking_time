...
# data
exp6_data = read.csv('processed_data/before_qc/exp6_data.csv') %>% mutate(experiment = 6)
...
ggplot(data_df %>% filter(!exclude, LT > 2), aes(x=trial_type, y=LT)) + geom_jitter(width = 0.03, alpha = 0.2) + 
  ...
  theme(strip.text.x = element_text(size = 20, colour = "black")) +ggtitle('exp6')


# main effect by trial type
ggplot(data_df %>% filter(!exclude, LT > 2), aes(x=block_num, y=LT)) + geom_jitter(width = 0.03, alpha = 0.2) + 
    stat_summary(fun.data = "mean_cl_boot", geom = "line", position = position_dodge(width = .1))  + 
  stat_summary(fun.data = "mean_cl_boot", geom = "pointrange", position = position_dodge(width = .1)) + xlab('Block number') + ylab('LT in s') + labs(color = 'fam dur') + theme_classic(base_size = 20) + 
