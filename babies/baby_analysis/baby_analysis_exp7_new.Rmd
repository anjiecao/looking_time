...
ggplot(plot_df, aes(x=test_type, y=LT, color = test_type)) + geom_jitter(alpha=0.2, position = position_dodge(width = .2)) + 
  ...
  stat_summary(fun.data = "mean_cl_boot", geom = "pointrange", size = 1, position = position_dodge(width = .1)) + xlab('Trial type') + ylab('LT in s') + labs(color = 'violation type') + theme_classic(base_size = 30) + 
...
model_1A <- lmer(log(LT) ~ block_num + (1|subject_num), data = model_df) 
model_1B <- lmer(log(LT) ~ block_num + is_violation + (1|subject_num), data = model_df) 
model_1C <- lmer(log(LT) ~ block_num + test_type  + (1|subject_num), data = model_df)
