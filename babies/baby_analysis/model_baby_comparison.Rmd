...
# plot 
ggplot(plot_df, aes(x = 5*fam_duration, y = value, color = test_type, group = test_type)) + geom_smooth(method = "lm", size = 3) +  geom_point(size = 3)  + ylab('Looking time (s)') + xlab('fam duration') + theme_classic(20) + scale_color_manual(values = c("blue", "red")) +  scale_x_continuous(breaks = c(0,5,10,15,20,30,40,45)) + facet_grid(~value_type) + xlab('Prior exposure to background (s)') + labs(color = 'Test type') + theme(legend.position = "bottom", legend.text = element_text(size = 25))
