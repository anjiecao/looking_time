...
model_data <- model_data %>%
  ...
  filter(all(0:9 %in% fam_duration)) %>%
  ungroup() %>%
  group_by(param_info) %>%
...
# plot 
ggplot(top_sim_result, aes(x = fam_duration, y = value, color = test_type)) + geom_point(size = 2) + geom_line() + facet_grid(~value_type) + ylab('LT') + xlab('fam duration') + scale_y_log10() + theme_classic(20)
