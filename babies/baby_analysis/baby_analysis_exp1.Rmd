...
## toggles
```{r}
# if TRUE, it takes output.xlsx in the data folder and concatenates everything from scratch
# if FALSE, it takes the full data table that's been stored and goes from there
reload_files = TRUE
...
if (reload_files) {
  # jspsych files
looking_list <- read_excel_allsheets(here::here("data", "exp1", "output.xlsx"))
...
complete_df <- complete_df %>% pivot_wider(names_from = coder, values_from = starts_with('look'))
  ...
  if (save_full_data){
    setwd('..')
    write.csv(complete_df, 'full_data_table.csv', row.names = FALSE)
  }
}  else if (!reload_files) {
      complete_df <- read.csv('data/full_data_table.csv')
}
...
ggplot(test_df, aes(x=block_type, y=normalized_LT, group=fam_duration)) + geom_point(alpha=0.2, position = position_dodge(width = .1)) + 
    stat_summary(fun.data = "mean_cl_boot", geom = "line", position = position_dodge(width = .1))  + 
  ...
  theme(strip.text.x = element_text(size = 14, colour = "black")) + theme(strip.text.x = element_text(size = 14, colour = "black"),
        axis.text=element_text(size=13),
        axis.title= element_text(size = 14)) +ggtitle('study 1')
