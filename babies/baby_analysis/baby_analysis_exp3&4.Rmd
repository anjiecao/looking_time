...
```{r}
common_columns = intersect(colnames(exp3_data), colnames(exp4_data))
...
data_df %<>% arrange(experiment, SubjectInfo.subjID, Trials.ordinal) %>% group_by(experiment, SubjectInfo.subjID, fam_or_test) %>% mutate(block_num = 1:n())
...
jspsych_list_exp4 <- lapply(jspsych_files_exp4[2:length(jspsych_files)], read.csv)
jspsych_list_exp4 <- lapply(jspsych_list_exp4, function(x) {x['success'] = NULL; x})
jspsych_df <- map_dfr(jspsych_list_exp4, ~ .x, .id = 'subjectid') 

consent_df <- jspsych_df %>% filter(nchar(response) > 5) %>% select(subjectid, response) %>% 
...
write.csv(data_df %>% mutate(experiment = case_when(experiment == 3 ~ 'A', experiment == 4 ~ 'B')) %>% filter(fam_or_test == 'test') %>% select(experiment, subject_num, block_num, test_type, fam_duration, LT, exclude, age, sex), 'infant_processed_data.csv')
```
...
ggplot(data_df %>% filter(fam_or_test == 'test', !exclude, experiment == 3), aes(x=test_type, y=LT, group=fam_duration, color = fam_duration)) + geom_jitter(alpha=0.2, position = position_dodge(width = .2)) + 
  ...
  theme(strip.text.x = element_text(size = 20, colour = "black")) +ggtitle('exp3')
...
# interaction model
model_1 <- lmer(log(LT) ~ age * poly(fam_duration,1) * test_type + trial_num + (1 | subject_num), data = test_df)
summary(model_1)

# ns. because of non-linear interaction

# dishabituation
model_2 <- lmer(log(LT) ~ fam_duration * test_type + trial_num + (1 | subject_num), data = test_df %>% filter(fam_duration >= 8) %>% mutate(fam_duration = scale(fam_duration)))
summary(model_2)
...
# age plot - fam pref vs age; novelty pref vs age - color dots by experiment


# stats (include experiment as a predictor )
