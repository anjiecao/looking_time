...
jspsych_df_exp3 <- map_dfr(jspsych_list_exp3, ~ .x, .id = 'subjectid') %>% mutate(experiment = 3) %>% 
  left_join(subject_mapping_exp3, by = c("subjectid" = "old_subject")) %>% select(-subjectid) %>% rename(subject_id = new_subject)
...
jspsych_df_exp4 <- map_dfr(jspsych_list_exp4, ~ .x, .id = 'subjectid') %>% mutate(experiment = 4) %>% 
  left_join(subject_mapping_exp4, by = c("subjectid" = "old_subject")) %>% select(-subjectid) %>% rename(subject_id = new_subject)

trial_df = rbind(jspsych_df_exp3,jspsych_df_exp4) %>% filter(stimulus_type == 'trial') %>% 
...
consent_df <- jspsych_df_exp4 %>% filter(nchar(response) > 5) %>% select(subjectid, response) %>% 
...
model_1a<- lmer(log(LT) ~ poly(fam_duration,1) * test_type +  block_num + (1 | subject_num), data = test_df)
summary(model_1a)
