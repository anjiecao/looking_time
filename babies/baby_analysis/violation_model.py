...
# df = pd.read_csv('processed_data/model_data/summarized_results_detailed_unity_infant.csv',
...
# df.to_csv("processed_data/model_data/sliced_model_data_unity_infant.csv")

df = pd.read_csv("processed_data/model_data/sliced_model_data_unity_infant.csv")
...
grouped_means = df.groupby(
    ['epsilon', 'mu_prior', 'v_prior', 'beta_prior', 'alpha_prior', 'stim_sequence', 'forced_exposure_max']
).agg(
    {'n_samples': 'mean', 'fam_duration': 'first', 'test_type': 'first'}
).reset_index()

grouped_means.to_csv('processed_data/model_data/grouped_model_means_new.csv', index=False)
...
#              df['forced_exposure_max'] == forced_exposure_max)
#         ]
...
#         plt.title('Forced exposure: ', forced_exposure_max)
#         plt.show()
