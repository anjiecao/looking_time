...
function getRandomSubarray(array, size, condition_order) {
  ...
  console.log(deviant_index)
  if (deviant_index != -1 ){
  // select the value for identity, remove this value plus its pair from possible options
  value = parseInt(tempArray[randomIndex])
  result[deviant_index]= tempArray.splice(randomIndex, 1)[0];
  if (value %2 == 0){
    pair = value-1;
  }
  else{
  ...
  let randomIndex_pair = tempArray.indexOf(pair.toString());
  tempArray.splice(randomIndex_pair,1);
...
function generate_all_block(condition_order,
    ...
    species = ['1', '2', '3', '4', '5', '6']
    species = getRandomSubarray(species, 4, condition_order)
