...
do
  ...
  # Calculate the new height and width (60% of the original height, 76% of the original width)
  new_height=$(awk -v h="$height" 'BEGIN{printf("%d", h*0.6)}')
  new_width=$(awk -v w="$width" 'BEGIN{printf("%d", w*0.76)}')

  # Calculate offsets (20% of the original height, 12% of the original width)
  y_offset=$(awk -v h="$height" 'BEGIN{printf("%d", h*0.2)}')
  x_offset=$(awk -v w="$width" 'BEGIN{printf("%d", w*0.12)}')
